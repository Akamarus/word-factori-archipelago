...
Generate a new 1.2.2 room. Choose `custom_level_set: core_campaign` for 30 custom levels or `custom_level_set: discovery_labs` for the recommended 40-level campaign. Launch **Word Factori Client**, then connect from the in-game panel, the regular client, or an `archipelago://` link. Received machine items and Progressive World Access copies update the mod's supported `levels.json`; return to save select or restart Word Factori after new unlocks. Sticker deliveries and ordinary reconnects do not require a reload.
...
Version 1.2.2 is a release-ready experimental public beta. The primary Windows 10 path is live-smoke tested; password-room and additional display permutations remain in the beta matrix. Only the bundled digest-verified `core_campaign` and `discovery_labs` manifests can be selected; arbitrary Workshop packs are not imported.
