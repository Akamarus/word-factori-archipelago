...
Version 1.2.1 is a release-ready experimental public beta. The primary Windows 10 path is live-smoke tested; password-room and additional display permutations remain in the beta matrix. Only the bundled digest-verified `core_campaign` and `discovery_labs` manifests can be selected; arbitrary Workshop packs are not imported.
