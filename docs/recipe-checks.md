...
An isolated native fixture verified journal recording and the 60-step flush behavior without modifying the live installation or save. Automated generation exercised 12 real Archipelago 0.6.7 cases covering `true`, `false`, and omitted values across both level sets and both goals. Exact location contracts, balanced fill, progression spheres, and Universal Tracker reconstruction passed. Generation time varies with the host and is recorded with the raw verification evidence rather than promised as a player-facing performance target.
