...
`b344fdd70986cc97bd37a8ba3659720cc2220d72611abf96c0214cc26a5e5fdc`
...
The corresponding unreleased player ZIP SHA256 is
`74b88399d06559ebbfdfde4fb397eef99811b28497a57ec3b2e0c59c9c9ff9b8`;
release verification passed source parity, JSON/index integrity, fresh recipe
derivation and data exclusions.

The raw Type-a-Word YAML boundary matrix passed **7/7** against that APWorld.
Null, true, mapping, scalar and disabled-but-present null list values were
rejected before OptionList coercion; a normal list and an omitted disabled
option were accepted. The result is retained at
`tests/output-typeword-verification/final-yaml/results.json`.
...
Controller commands: `run_matrix.py` and `verify_tracker.py`, using the absolute
output directory `project/probe-tools/tw-final-matrix`. Raw reports are retained
there as `results.json` and `tracker-results.json` alongside per-case logs.
Generation logs contained only the pre-existing
