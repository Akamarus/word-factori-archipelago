...
The 1.5.1 patch additionally fixes duplicate-entry recipe normalization and pins bundled recipes in AP sessions. Vanilla online recipe updates remain enabled outside AP. Isolated native regression tests reproduce the failing recipe shape without reading player saves, then verify repeated refreshes and vanilla/AP transitions. Live save-loading acceptance on Windows and Proton is still needed.
...
1. Extract **word-factori-archipelago-1.5.1.zip** and close Word Factori and Archipelago.
...
4. For a new playtest, generate a **new room** using a bundled example YAML. There are no integration-mode or campaign-layout selectors. Use a fresh empty save and new 1.5.1 room; old-room compatibility is not promised.
